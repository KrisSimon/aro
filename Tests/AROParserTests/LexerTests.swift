...
struct TokenKindTests {
    ...
    func testKeywordDescriptions() {
        ...
        #expect(TokenKind.while.description == "while")
        #expect(TokenKind.repeat.description == "repeat")
        #expect(TokenKind.until.description == "until")
        #expect(TokenKind.break.description == "Break")
        #expect(TokenKind.continue.description == "Continue")
    ...
    func testIsStatementKeyword() {
        ...
        #expect(TokenKind.while.isStatementKeyword == true)
        #expect(TokenKind.repeat.isStatementKeyword == true)
...
struct LexerTokenizationTests {
    ...
    func testIterationKeywords() throws {
        let tokens = try Lexer.tokenize("for each in at while repeat until Break Continue")

        #expect(tokens[0].kind == .for)
        #expect(tokens[1].kind == .each)
        #expect(tokens[2].kind == .in)
        #expect(tokens[3].kind == .atKeyword)
        #expect(tokens[4].kind == .while)
        #expect(tokens[5].kind == .repeat)
        #expect(tokens[6].kind == .until)
        #expect(tokens[7].kind == .break)
        #expect(tokens[8].kind == .continue)
    }
